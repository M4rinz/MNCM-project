...
\section{Stimatori per \texorpdfstring{$P$}{P}}
	...
	\begin{frame}
		...
		che $\mathbb{E}\qty[\transpose{\vb*{n}_t}\mid\vb*{n}_{t-1}]=\transpose{\vb*{n}_{t-1}}P$.}
		\smallskip
		...
		\onslide<4->{L'insieme di relazioni $\transpose{\vb*{n}_t}\approx\transpose{\vb*{n}_{t-1}}P\quad\forall\,t\in\qty{2,\dots,T}$
        ...
        \onslide<5->{Considerazioni (cfr. Appendice~\hyperlink{frame:proprieta_P_CLS:appendice}{\faHandPointRight}):}
        \begin{enumerate}
            \item<5-> \textcolor{red}{invertibilità di $\transpose{X}X$}
            \item<6-> $\hat{P}_{\textup{CLS}}\cdot e=e$
            \item<7-> Non è vero che $\hat{P}_{\textup{CLS}}\ge 0$, in generale.
        \end{enumerate}
	...
	\begin{frame}
		...
		$\mathbb{E}\qty[\epsilon_t]=0$, $\Var{\epsilon_t}<\infty,\epsilon_t\independent\vb*{n}_t\quad\forall\,t\in[T]$,
		...
		\[
			\hat{P}_{\textup{CLS}}=\qty(\hat{\Lambda}_t+\Var{\epsilon_t})^{-1}\hat{\Lambda}_{t,t+1}\xcancel{\overset{T\to\infty}{\longrightarrow}P}
		\]
...
\section*{Appendice}
	...
	\begin{frame}
		...
		\end{align*}
		Ovvero $\mathbb{E}\qty[\transpose{\vb*{n}_t}\mid\vb*{n}_{t-1}]=\transpose{\vb*{n}_{t-1}}P$.}
    ...
    \begin{frame}
        {\hypertarget{frame:proprieta_P_CLS:appendice}{Proprietà di $\hat{P}_{\textup{CLS}}$}}

        \begin{enumerate}
            \item Si suppone che $T\gg S$, dunque $\transpose{X}X$ è invertibile $\iff\rank{X}=S$. 
            \[
                X=\sum_{m=1}^N
                 \left(\begin{array}{c|c|c}
				    V_1^{(m)} & \dots & V_{T-1}^{(m)} \\  
                \end{array}\right)^{\top}
                \equiv\sum_{m=1}^N\mathcal{V}^{(m)}, \quad\mathcal{V}^{(m)}\in\qty{0,1}^{(T-1)\times S}
            \]
            \textbf{Idea}: $P$ è ergodica, quindi se $T$ è grande abbastanza, qualsiasi stato è visitato. Se qualsiasi stato fosse visitato, $\mathcal{V}^{(m)}$ avrebbe $e_1,\dots,e_S$ tra le sue righe; dunque si avrebbe $\rank\mathcal{V}^{(m)}=S\quad\forall m\in[N]$
            \item Osserviamo che $X e=Y e=N\cdot e$. Dunque
            \[
                \hat{P}_{\textup{CLS}} e=e\iff \transpose{X}Ye=\transpose{X}Xe\iff N\cdot\transpose{X}e=N\cdot\transpose{X}e
            \]
            \item Siano $X\coloneqq\mqty(5 & 5 \\ 9 & 1), Y\coloneqq\mqty(9 & 1\\1 & 9)$. 
            Si ottiene $\hat{P}_{\textup{CLS}}=\frac{1}{10}\mqty(-1 & 11 \\ 19 & -9)\not\ge 0$.
        \end{enumerate}
